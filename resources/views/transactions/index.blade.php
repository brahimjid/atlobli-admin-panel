    ...
    <div class="page-wrapper">
        <div class="row page-titles">
            <div class="col-md-5 align-self-center">
                <h3 class="text-themecolor">{{ trans('lang.wallet_transaction_plural') }} <span
                        class="userTitle"></span></h3>
            </div>
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="menu-tab d-none storeMenuTab">
                            <ul>
                                <li>
                                    <a href="{{ route('restaurants.view', $id) }}"
                                       id="basic">{{ trans('lang.tab_basic') }}</a>
                                </li>
                                <li>
                                    <a href="{{ route('restaurants.foods', $id) }}"
                                       id="tab_foods">{{ trans('lang.tab_foods') }}</a>
                                </li>
                                <li>
                                ...
                                <li>
                                    <a href="{{ route('payoutRequests.restaurants.view', $id) }}"
                                       id="payout_request">{{ trans('lang.tab_payout_request') }}</a>
                                </li>
                                <li>
                                ...
                                <li id="subscription_plan">
                                    <a href="{{ route('vendor.subscriptionPlanHistory', $id) }}"
                                       class="active">{{ trans('lang.subscription_history') }}</a>
                                </li>
                                <li>
                                    <a href="{{ route('restaurants.advertisements', $id) }}"
                                       id="advertisements">{{ trans('lang.advertisement_plural') }}</a>
                                </li>
                                <li>
                                    <a href="{{ route('restaurants.deliveryman', $id) }}"
                                       id="deliveryman">{{ trans('lang.deliveryman') }}</a>
                                </li>
                            </ul>
                        </div>
...
<script src="https://cdn.jsdelivr.net/npm/@fancyapps/ui@5.0/dist/fancybox/fancybox.umd.js"></script>
    ...
    <script>
        ...
        $(document).ready(function () {
         ...
         function validateTransaction(new_transaction_id) {
           database.collection('wallet').doc(new_transaction_id).get().then(function (trx){
               var transaction = trx.data();
               Swal.fire({
                   title: "{{ trans('lang.confirmation') }}",
                   text: "{{ trans('lang.validate_transaction_alert') }}",
                   icon: "warning",
                   showCancelButton: true,
                   confirmButtonColor: "#3085d6",
                   cancelButtonColor: "#d33",
                   confirmButtonText: "{{ trans('lang.yes') }}",
                   cancelButtonText: "{{ trans('lang.no') }}"
               }).then((result) => {
                   if (result.isConfirmed) {
                       var transactionId = transaction.id;
                       var transactionStatus = transaction.payment_status;
                       database.collection('users').doc(transaction.user_id).get().then(function (usx){
                           var user = usx.data();
                           if (transactionStatus == 'waiting') {
                               var final_wallet_balance = Number(user.wallet_amount) + Number(transaction.amount);
                               // alert(final_wallet_balance);
                               // return;
                               database.collection('wallet').doc(transactionId).update({
                                   payment_status: 'success'
                               }).then(function () {
                                   database.collection('users').doc(transaction.user_id).update({
                                       'wallet_amount': Number(final_wallet_balance),
                                   }).then(function (){
                                       Swal.fire(
                                           "{{ trans('lang.success') }}",
                                           "{{ trans('lang.successfully_validated') }}",
                                           "success"
                                       );

                                        $('#walletTransactionTable').DataTable().ajax.reload(null, false);
                                   }).catch(function (){
                                       Swal.fire(
                                           "{{ trans('lang.error') }}",
                                           "{{ trans('lang.error_validating') }}",
                                           "error"
                                       );
                                   });
                               });
        ...
        function declineTransaction(new_transaction_id) {
            database.collection('wallet').doc(new_transaction_id).get().then(function (trx){
            var transaction = trx.data();
            Swal.fire({
                title: "{{ trans('lang.confirmation') }}",
                text: "{{ trans('lang.decline_transaction_alert') }}",
                icon: "warning",
                showCancelButton: true,
                confirmButtonColor: "#3085d6",
                cancelButtonColor: "#d33",
                ...
                if (result.isConfirmed) {
                    ...
                    database.collection('wallet').doc(transactionId).update({
                        ...
                        Swal.fire(
                            "{{ trans('lang.success') }}",
                            "{{ trans('lang.transaction_declined_successfully') }}",
                            "success"
                        );
                        $('#walletTransactionTable').DataTable().ajax.reload(null, false);
                        ...
                        Swal.fire(
                            "{{ trans('lang.error') }}",
                            "{{ trans('lang.transaction_decline_failed') }}",
                            "error"
                        );
                    });
                }
        ...
        async function buildHTML(val) {
            ...
            if (val.payment_method) {
                if (val.payment_method == "Stripe" || val.payment_method == "stripe") {
                    ...
                    image = '{{ asset('images/atlobli_wallet.png') }}';
                    payment_method = '<img alt="image" src="' + image + '" >';
                 ...
                 else if (val.payment_method=='bankily'){
                     image = '{{asset('rim_wallets/bankily.png')}}';
                    payment_method = '<img alt="image" src="' + image + '" >';

                }
                 else if (val.payment_method=='sedad'){
                     image = '{{asset('rim_wallets/sedad.png')}}'
                    payment_method = '<img alt="image" src="' + image + '" >';

                } //
                 else if (val.payment_method=='masrevi'){
                     image = '{{asset('rim_wallets/masrevi.png')}}';
                    payment_method = '<img alt="image" src="' + image + '" >';

                }
                 else if (val.payment_method=='bimBank') {
                    image = '{{asset('rim_wallets/bimBank.png')}}';
                    payment_method = '<img alt="image" src="' + image + '" >';
            ...
            } else if (val.payment_status == 'waiting') {
                html.push('<span class="badge badge-warning"><span>' + val.payment_status + '</span></span>');
            } else if (val.payment_status == 'declined') {
                html.push('<span class="badge badge-danger text-white"><span>' + val.payment_status + '</span></span>');
            ...
            if (val.is_mr) {
                actions =
                    `<div class="btn-group">`;
                if(val.payment_status == 'waiting') {
                    actions += `
                    <button onclick="validateTransaction('${val.id}')" class="btn btn-sm btn-success mr-1 ">
                        <span class="fa fa-check"></span>
                    </button>
                    <button onclick="declineTransaction('${val.id}')" class="btn btn-sm btn-danger">
                    ...
                    actions += `
                    <button onclick="declineTransaction(val)" class="btn btn-sm btn-success mr-1 ">
                        <span class="fa fa-check"></span>
                    </button>
                    `;
                }
